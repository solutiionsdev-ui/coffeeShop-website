...
5. **CPU decode / parse.** Geometry decode, normal estimation, PCA fits, buffer
   ...
   *seconds* (a measured 3.9 s for 50k plane fits), and it lands while the
   ...
   Workers.
6. **The page's own paint pipelines — not the scene at all.** Chrome compiles a
   GPU pipeline per new combination of CSS gradient, mask, blend, clip or
   layer opacity the first time it is drawn: 190–400 ms each, first visit only.
   A long frame with `scripts: []` is this, not JS. Measure it cold, read the
   pipeline names from the trace, warm them under the loader — method in
   `obsidian/workflows/optimize-3d-scene.md` → "§3 reaches past the canvas".
