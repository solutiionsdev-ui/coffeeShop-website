...
## §3 reaches past the canvas: the page's own GPU pipelines

A scroll freeze on a page with a scene is not necessarily the scene. In brewns
(2026-09-12) every first-visit freeze between the hero and the next block came
from Chrome compiling **paint** pipelines — swirl masks, a scrim, translucent
layers — the first time each was drawn. The fix lives in the preloader
(`GpuWarmup`, [[decisions-log]] ADR-0023); the method is what is reusable.

**Tell the two apart first.** A long animation frame whose Long Animation Frame
entry has `scripts: []` and a few ms of render is not the main thread. In a
trace, a paint-pipeline compile is one `CommandBuffer::addRenderPass` on the GPU
thread blocked on one `DawnPlatformImpl::RunWorkerTask` of 150–400 ms. A WebGL
one is `GLES2Implementation::GetProgramiv` → `WaitForCmd` on the renderer main
thread — the program was linked but first used, and checked, mid-scroll.

**Measure cold, or measure nothing.** Once compiled, a pipeline is cached in the
profile *and* in macOS's Metal cache, which survives fresh profiles: after two
or three runs the freezes vanish and the page looks fixed. Use Playwright's
Chrome for Testing (a separate app bundle — never touch the user's Chrome) and
wipe its Metal cache before every run:

```sh
rm -rf "$(getconf DARWIN_USER_CACHE_DIR)"/com.google.chrome.for.testing*/com.apple.metal
```

**Read the pipeline names.** Trace with `disabled-by-default-skia.shaders` (and
`gpu.angle` for WebGL). Every `createGraphicsPipeline` event carries the
pipeline key in `args.desc`, e.g.
`RP((BGRA8+D16 x1).rgba) + CoverBoundsRenderStep[NonAAFill] + LocalMatrix[ConicalGradient4…]+Dither SrcOver`.
Put a `performance.mark()` at scroll start (and one when the loader leaves),
place every creation on that timeline, and the list of keys created *during the
scroll* is the to-do list. The trace runs to 400–650 MB — stream it line by
line; it will not fit in one JavaScript string.

**Reading a key.** `RP(…)` is the render target — `x1` single-sampled, `x4->1`
multisampled (a tile with one even-odd path goes multisampled whole). The render
step says the shape (`CoverBounds` = a rect on whole pixels, `AnalyticRRect` =
antialiased or rounded, `PerEdgeAAQuad` = a turned or scaled compositor quad).
`BlendCompose[…, AlphaOnlyPaintColor, SrcIn]` is a layer at partial opacity;
`ImageShaderClamp` vs `HardwareImage` is how the compositor samples a layer's
tiles; `AnalyticClip` is a non-rectangular clip.

**Attribute before you sample.** Hiding elements moves a shared pipeline to the
next element that needs it rather than removing it. Timestamping the first
moment each element takes a partial inline opacity (a `MutationObserver` on
`style`) and lining that up with the creation time names the element in one run.
