...
## Shader specs — `docs/opalesce.md`

The hero's animated background is a plain WebGL2 fragment shader with no
dependency of its own. Its specification lives at **`docs/opalesce.md`** in the
repo root rather than in this vault, because it is a vendor document reproduced
verbatim — the GLSL, the engine and the parameter reference are the source of
truth for `src/components/hero/hero-gradient.tsx`, and editing either the shader
or the palette anywhere else produces mud. See
[[components/common|the component catalog]] for how it is wired in.

## Fonts

| Family | Source | Weights | Used for |
|--------|--------|---------|----------|
| Onest | `next/font/google` | variable | `--font-sans`, the starter default |
| Geist | `next/font/google` | 600, 700 | `--font-display` — every headline and the bold copy |
| Space Mono | `next/font/local` | 400 | `--font-mono` — nav, eyebrows, info rows |
| Allura | `next/font/local` | 400 | `--font-script` — the hero's "moment" |

> [!note] Why Geist is not local like the other two
> `src/fonts/Geist-Bold.ttf` is a **static** instance: no `fvar` axis,
> `usWeightClass` 700. Block 4's headline needs SemiBold (600), which cannot be
> synthesised from it, so the family comes from Google where both weights exist.
> The local file is now unused.
